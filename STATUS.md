...
- 컷 강도 슬라이더 단일화. 실측 결과 min_silence(쉼 감도)는 효과 약하고(0.1↔0.3에서 2s), pad(말 끝 여유)가 컷량을 좌우(0↔0.2에서 11s). 헷갈리는 슬라이더 2개를 '컷 강도' 1개로 합침(오른쪽=바짝, pad로 매핑, min_silence 0.15 고정). 강도 0↔100%에서 보존 35.0↔20.3s로 효과 뚜렷. 참고: 마침표·1줄 미적용은 직전 커밋 후 서버 미재시작(옛 코드)이 원인 — 재기동 후 신선 드래프트에서 끝마침표 0개·type=text 확인.
