...
- 정확도 2차 라운드. (A) 용어 사전: 설정 화면(직접 입력 + 엑셀/CSV 일괄), initial_prompt(첫 윈도우) + apply_glossary 전 구간 받아쓰기 교정(음절 lev1 + 자모 최근접: 잣말→잔말, 굿말→군말), ASR 캐시 키에 모델·용어 반영. 잔말 단어 편집, 고정밀(large-v3) 토글. (B) Silero VAD 컷 경계 보정: whisper 단어 끝 '이른 편향'을 실제 음성 에너지로 보정(ffmpeg 직접 PCM, torchcodec 불필요) — 9분 실영상 65→56구간(가짜 컷 병합), +15.5s 꼬리 보존, 경계 한도 위반 0. (C) 소소: 자막 어미/조사 경계 자연 분할, NG 앞/뒤 테이크 비교(▶뒤·앞 제거·뒤 제거), 회귀 테스트 20건(tests/test_core.py), favicon(Dock 아이콘 동일). LLM 교정(Ollama)은 보류.
...
- e14cf08(검토화면 슬라이더 '컷 강도' 1개로 단일화) 되돌림 → 슬라이더 2개('이보다 긴 쉼만 자르기' + '말 끝 여유')로 복귀(사용자 요청). 안전 revert(공개 history 유지). 그 외 변경(반복컷·마침표·1줄·무음튜닝)은 유지.
